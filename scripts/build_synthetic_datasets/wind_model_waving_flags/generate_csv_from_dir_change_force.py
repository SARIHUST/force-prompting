...
def analyze_video(video_path, backgrounds_json):
    ...
    fname = os.path.basename(video_path) # e.g. 'flag_sample_1821.9_11534.2_0.0_298.7_118.7_0.0_background_acoustical_shell_4k.mp4'
    force_info = fname.split("_0.0_background_")[0] # e.g 'flag_sample_1821.9_11534.2_0.0_298.7_118.7'
    background_name = fname.split("_0.0_background_")[1].split(".mp4")[0] # e.g. 'garden_nook_4k'

    force_info_split = force_info.split("_")
    # e.g. force_info_split = ['flag', 'sample', '1821.9', '11534.2', '0.0', '298.7', '118.7']
    speed1 = float(force_info_split[2])
    speed2 = float(force_info_split[3])
    angle1 = float(force_info_split[5])
    angle2 = float(force_info_split[6])
...
def main():
    ...
    if args.file_type == "video":
        ...
        for path in tqdm(files):
            ...
            if result:
                min_speed = min(min(result["wind_speed_1"], result["wind_speed_2"]), min_speed)
                max_speed = max(max(result["wind_speed_1"], result["wind_speed_2"]), max_speed)
                results.append(result)

        results.sort(key=lambda x: x['wind_speed_1'])   # use speed1 for sorting
        ...
        with open(args.output_path, 'w', newline='') as csvfile:
            if args.file_type == "video":
                fieldnames = ['video', 'wind_speed_1', 'wind_speed_2', 'wind_angle_1', 'wind_angle_2', 'frame', 'fps', 'seconds', 'caption']
            elif args.file_type == "image":
            ...
            for result in results:
                ...
                if NORMALIZE_WIND_SPEED:
                    result["wind_speed_1"] = (result["wind_speed_1"] - min_speed) / (max_speed - min_speed)
                    result["wind_speed_2"] = (result["wind_speed_2"] - min_speed) / (max_speed - min_speed)
                    result["wind_speed_1"] = "{:.4f}".format(result["wind_speed_1"])
                    result["wind_speed_2"] = "{:.4f}".format(result["wind_speed_2"])
